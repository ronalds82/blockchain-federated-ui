...
contract TrainingStatusManager {
    ...
    enum Status { NONE, WAITING_FOR_PARTICIPANTS, START_TRAINING, TRAINING_COMPLETED }
    ...
    function updateStatus(Status _newStatus) public {
        require(uint(_newStatus) <= uint(Status.TRAINING_COMPLETED), "Invalid status");
    ...
    function getStatus() public view returns (string memory) {
        string[4] memory statusStrings = ["NONE", "WAITING_FOR_PARTICIPANTS", "START_TRAINING", "TRAINING_COMPLETED"];
